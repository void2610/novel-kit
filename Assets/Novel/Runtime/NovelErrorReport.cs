...
namespace Novel.Runtime
{
    ...
    {
        public static NovelErrorInfo Describe(string scenarioKey, Exception ex, int sayNumber = 0, string lastSayText = "")
        {
            var backtrace = TryGetRubyBacktrace(ex);
            var detail = string.IsNullOrEmpty(backtrace) ? ex.ToString() : backtrace!;
            return new NovelErrorInfo(scenarioKey, ex.Message, detail, ex, sayNumber, lastSayText);
        }
