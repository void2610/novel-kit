...
- **対象外**: スプレッドシート / CSV / Excel によるシナリオ読み込み（void-red 方式）。
  ...
  CG 鑑賞ギャラリー・シーン回想メニュー等の**ゲーム全体メタ機能**も対象外（ノベルパート外の画面であり game 側の責務）。
...
1. [実行モデル](/design/decisions/execution-model.md): 前進専用 + チェックポイント割り切り（セーブは `PlayAsync` 境界のみ。2026-06-14 にリプレイ前提から格下げ）。
...
5. [決定性コントラクト](/design/decisions/determinism-contract.md): 当面 moot（実行モデルが前進専用へ格下げされリプレイ自体が消失。入力履歴の day1 記録も行わない）。
