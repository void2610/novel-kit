...
[実行モデル](/design/decisions/execution-model.md) のリプレイ前提では記録した選択/フラグ履歴を `.rb` 再実行で
再現するため、シナリオ編集で行の index ずれ等が起き得る。これは **コマンドスキーマとは無関係**で、save-anywhere を
実装する段階で「履歴のシナリオ版数チェック → 不一致なら近いチェックポイントへフォールバック」等で扱う。
