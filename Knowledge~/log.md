...
## 2026-07-05
* **Update**: セーブの JSON serde をライブラリが持つ形へ。[セーブ粒度](/design/decisions/save-snapshot.md) の当初帰結「ライブラリはセーブ形式を持たず game 所有」を改めるが、**永続化(保存する行為)は引き続き game 所有**で、novel-kit はセーブデータの表現までを渡す。主用途(標準): game は自前 JSON セーブ機構を持つ前提で、`Novel.Runtime` に公開クラス `NovelSaveData`(`[Serializable]`・プレーンフィールド・`From`/`ToSnapshot`・自前 serde でネスト可)と文字列版 `NovelSaveSerializer`(内部で `NovelSaveData` + Unity 標準 `UnityEngine.JsonUtility`・決定的出力・破損耐性)を追加。game は自前 `ISaveStore` を実装しこれらで snapshot を自分の save に畳み込む。副次用途(任意・非標準): novel-kit に永続まで丸ごと任せたい小規模ケース向けに `JsonSaveStore : ISaveStore`(serde を内部で完結・隠蔽) + `INovelSaveBlobStore`(書き込み先だけ game 供給) + `PlayerPrefsSaveBlobStore`(Novel.View) + DI `RegisterNovelJsonSave<TBlob>()`。粒度/境界(永続は IStateStore の Capture・境界は PlayAsync の狭間)は不変。color-recollection のセーブ実装(フラグ/既読の game 側直列化)を参考にデータ表現をライブラリへ集約。EditMode `NovelSaveSerializerTests` 追加(Unity 未起動の本環境ではテスト未実行)。
