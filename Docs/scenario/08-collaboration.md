...
| **STEP 4** | コミット → push → PR を出し、リンクをチームに報告（提出） | GitHub Desktop + チャット |
...
2. ブランチ名を入力する。**内容が分かる名前**を、小文字の英単語をハイフン（`-`）で
   つないで付ける（例: `scenario-chapter2`、`fix-typo-chapter1`）。細かい決まりはない。
   スペースは使えない。日本語も文字化けの元になるので半角英数字にする。

やってはいけない付け方:

| 悪い例 | 理由 |
|---|---|
| `test`・`sagyou`・`branch1` | 何の作業か分からず、次の作業でも同じ名前を使い回したくなる。「1 つの作業 = 1 つのブランチ」が崩れる |
| `yamada`・`0807` | 誰が・いつ作業したかは Git が自動で記録している。ブランチは「何の作業か」で区別するもの |
...
> ただし例外が 1 つ: 自分が追加・編集したファイルと**同じ名前の `.meta` ファイルは自分の変更とセット**
> なので、必ず一緒に提出する（Unity の管理ファイルで、これを外すと他の人の環境でエラーになる）。
...
| 悪い例 | 理由 |
|---|---|
| `テスト`・`修正`・`いろいろ変更` | 何を変えたのか後から分からない。「どの変更で表示がおかしくなった？」を探すとき、履歴が全部これだと総当たりになる |
| `8/7の作業`・`山田` | 日付と作者はコミットに自動で記録されるので、書いても情報が増えない |
...
3. 緑の **`Create pull request`** ボタンを押す。

![GitHub の PR 作成画面でタイトルと説明を書いて Create pull request を押す](./images/collab-create-pr.png)

4. 作成後に表示される PR ページの URL（ブラウザのアドレス欄）をコピーし、
   **Discord などチームの連絡チャンネルにリンクを貼ってエンジニアに報告する**。
   例: 「2章の教室シーンの PR を出しました。レビューお願いします → (リンク)」
   GitHub の通知だけでは気づかれないことがあるので、報告までやって提出完了 🎉
